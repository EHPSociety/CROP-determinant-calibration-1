data <- read.csv("survey_926864_R_data_file.csv", quote = "'\"", na.strings=c("", "\"\""), stringsAsFactors=FALSE, fileEncoding="UTF-8")
...
data[, 16] <- factor(data[, 16], levels=c(1,2,3,4,5),labels=c("High school (secondary education)", "Polytechnical education", "3", "A bachelor\'s degree", "A master\'s degree or higher"))
